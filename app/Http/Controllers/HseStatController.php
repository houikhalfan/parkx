...
class HseStatController extends Controller
{
    public function show()
    {
        return Inertia::render('Statistique/StatistiquesPage');
    }
    ...
    public function store(Request $r)
    {
        ...
        // Combine user's chosen date with current time
        if (!empty($data['date'])) {
            // Parse the user's chosen date
            $userDate = Carbon::parse($data['date']);
            
            // Create a new datetime with user's date but current time
        ...
        $statistics = HseStat::create(array_merge($data, [
            'user_id'                         => Auth::id(),
            'user_type'                       => 'user',
            'date'                            => $finalDateTime, // User's date + current time
        ...
        foreach ($admins as $admin) {
            $admin->notify(new \App\Notifications\StatisticsSubmitted($statistics, $user));
        }

        return redirect()->route('statistiques.history')
    ...
    public function history()
    {
        ...
        return Inertia::render('Statistique/StatistiquesHistory', [
